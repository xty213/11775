...
for event in P001 P002 P003; do
  ...
  python scripts/train_svm.py $event "features/" 32700 sift_med/svm.$event.model || exit 1;
  # apply the svm model to *ALL* the testing videos;
  # output the score of each testing video to a file ${event}_pred 
  python scripts/test_svm.py sift_med/svm.$event.model "features/" 32700 sift_med/${event}_pred || exit 1;
