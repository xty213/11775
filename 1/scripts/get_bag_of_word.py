...
def writeBoWFile(listPrefix, eventNum, showClipID = False):
    with open('%s/%s_%d.csv' % (BOW_PATH, listPrefix, eventNum), 'w+') as bowFile:
        with open("%s/%s_%d.list" % (LIST_PATH, listPrefix, eventNum), 'r') as f:
            # print table head
...
for eventNum in EVENT_LIST:
    with open("%s/train_%d.list" % (LIST_PATH, eventNum), 'r') as f:
        for line in f:
            clipID = line.split(',')[0]
            fileString = "%s/HVC%s.ctm" % (ASR_PATH, clipID)
            if not os.path.exists(fileString):
