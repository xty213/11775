...
def writeBoWFile(listPrefix, eventNum, showClipID = False):
    with open(BOW_PATH + '%s_%d.csv' % (listPrefix, eventNum), 'w+') as bowFile:
        with open(LIST_PATH + "%s_%d.list" % (listPrefix, eventNum), 'r') as f:
            ...
            for line in f:
                ...
                if not os.path.exists(ASR_PATH + "HVC%s.ctm" % clipID):
                    continue
                with open(ASR_PATH + "HVC%s.ctm" % clipID, 'r') as asr:
                    for rec in asr:
                        word = rec.split(" ")[4]
...
for eventNum in EVENT_LIST:
    with open(LIST_PATH + "train_%d.list" % eventNum, 'r') as f:
        for line in f:
            clipID = line.split(',')[0]
            if not os.path.exists(ASR_PATH + "HVC%s.ctm" % clipID):
                continue
